			...
			try {
				...
				if (importedConfig) {
					loadConfig({
						...
						description:
							importedDesc ?? importedConfig.description ?? "",
						stages: importedConfig.stages ?? [],
					});
				}

				// Only call setState if we have specific layout data to override the defaults generated by loadConfig
				if (importedNodes || importedEdges || importedViewport) {
					useDesignerStore.setState({
						...(importedNodes ? { nodes: importedNodes } : {}),
						...(importedEdges ? { edges: importedEdges } : {}),
						...(importedViewport
							? { viewport: importedViewport }
							: {}),
					});
