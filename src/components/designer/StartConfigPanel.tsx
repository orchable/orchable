    ...
    const rootStages = useMemo(() => {
        ...
        return nodes
            ...
            .map(n => ({
                ...
                contract: (n.data as any).contract
            }));
    }, [nodes, edges]);

    const validation = useMemo(() => {
        if (inputData.mode === 'tsv' || rootStages.length === 0 || !inputData.fileName) return { isValid: true, problems: [] };
        ...
        const perTaskFields = inputData.fieldSelection.perTask.map(f => f.split('.').pop() || f);

        // SMART VALIDATION: If 'input_data' object is selected, unroll its keys into selectedFields
        const unrolledFields: string[] = [];
        if (perTaskFields.includes('input_data') && inputData.jsonAnalysis?.sampleTasks?.[0]?.input_data) {
            const inputDataObj = inputData.jsonAnalysis.sampleTasks[0].input_data;
    ...
    return (
        <Card className="h-full border-none rounded-none shadow-none flex flex-col">
            <CardHeader className="flex-shrink-0">
                ...
                <CardDescription>Configure basic metadata and runtime input data</CardDescription>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto space-y-6 pb-20">
                ...
                <div className="space-y-4">
                        ...
                        <div className="border-2 border-dashed border-muted-foreground/20 rounded-lg p-6 text-center hover:border-primary/50 hover:bg-primary/5 transition-all">
                            {isParsing ? (
                                ...
                                <>
                                    ...
                                    <p className="text-xs font-medium mb-1">Click to upload {inputData.mode.toUpperCase()}</p>
                                    <p className="text-[10px] text-muted-foreground">or drag and drop</p>
                                </>
                            )}
                        </div>
                    </div>
                    ...
                    {inputData.fileName && (
                        <div className="p-3 rounded-md bg-success/5 border border-success/20">
                            <div className="flex items-center gap-2">
                                {inputData.mode === 'json' ? <FileJson className="w-4 h-4 text-success" /> : <FileSpreadsheet className="w-4 h-4 text-success" />}
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-[11px] truncate">{inputData.fileName}</p>
                                    <p className="text-[9px] text-muted-foreground">
                                        {inputData.mode === 'json' ? `${inputData.jsonAnalysis?.sampleTasks?.length || 0} tasks` : `${inputData.syllabusData.length} rows`} detected
                                    </p>
                                </div>
                                <Button
                                    variant="ghost"
                    ...
                    {inputData.mode === 'json' && inputData.jsonData && inputData.jsonAnalysis && (
                        <div className="space-y-4 pt-2">
                            ...
                            {!validation.isValid && (
                                <Alert variant="destructive" className="py-2 px-3">
                                    ...
                                    <AlertTitle className="text-[11px] font-bold">Incomplete Data Mapping</AlertTitle>
                                    <AlertDescription className="text-[10px] space-y-2 mt-1">
                                        {validation.problems.map((problem, i) => (
                                            <div key={i}>
                                                ...
                                                <p>Missing: <span className="font-mono">{problem.missing.join(', ')}</span></p>
                                            </div>
                                        ))}
                                        <Button
                                            size="sm"
                ...
                <FieldMappingDialog
                    ...
                    contract={null}
                    availableFields={inputData.jsonAnalysis ? [
                        ...inputData.jsonAnalysis.sharedFields.map((f: any) => f.path),
                        ...inputData.jsonAnalysis.perTaskFields.map((f: any) => f.path)
                    ] : []}
                    mapping={inputData.fieldMapping}
