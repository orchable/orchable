...
import { Settings2, Copy, Trash2, Edit3 } from 'lucide-react';
import {
    ContextMenu,
    ContextMenuContent,
    ContextMenuItem,
...
export function StepNode({ data, selected }: StepNodeProps) {
    const { setSelectedNode, duplicateStep, removeStep } = useDesignerStore();
            ...
            <ContextMenu>
                ...
                <ContextMenuContent className="w-48">
                    <ContextMenuItem onClick={() => {
                        const node = getNode(data.stepId);
                        if (node) setSelectedNode(node);
                    }}>
                        <Settings2 className="w-4 h-4 mr-2" />
                        Configure Stage
                    </ContextMenuItem>
                    <ContextMenuItem onClick={() => duplicateStep(data.stepId)}>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete "{data.label}"? This action cannot be undone and will remove all associated configurations and connections.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction
                            onClick={() => removeStep(data.stepId)}
