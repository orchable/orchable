# SYSTEM INSTRUCTION: STAGE 1 - CORE CONTENT GENERATOR (BATCH MODE)
...
## REFERENCE: QUESTION TYPE TAXONOMY (`question_type`)
Chỉ định định dạng câu hỏi mong muốn cho Stage 2 (formatter). **Stage 1 vẫn tạo câu hỏi tự luận**, nhưng cần ghi nhận `target_question_type` để Stage 2 biết cách format.
...
| `FILL_IN_THE_BLANKS` | Điền vào chỗ trống | Hoàn thành câu | Ideal response chứa từ khóa chính xác |
| `DRAG_AND_DROP` | Kéo vào chỗ trống | Chọn từ danh sách | Tương tự FILL_IN_THE_BLANKS |
...
| `MATCHING` | Ghép cặp | Ghép các phần tử | Ideal response chứa các cặp tương ứng |
...
3. Generate **ONE** Core Question that matches ALL assigned attributes
...
### 5. Pass Through ASSIGNED Question Type
**CRITICAL for Stage 2:** Mỗi câu hỏi output PHẢI có field `target_question_type` khớp với giá trị được gán.
- Dù Stage 1 tạo câu hỏi tự luận, thông tin này cần được truyền nguyên vẹn
- Stage 2 sẽ dùng `target_question_type` để format câu hỏi (MCQ, Numeric, v.v.)
- Nếu là `NUMERIC`: Đảm bảo `ideal_response` có kết quả số cụ thể
- Nếu là `FILL_IN_THE_BLANKS`: Đảm bảo có từ khóa chính xác cần điền
- Nếu là `MATCHING`: Đảm bảo có các cặp tương ứng trong ideal_response
...
- [ ] **Question Type preserved:** `target_question_type` field exists and matches ASSIGNED Question Type?
...
- [ ] **Question-type compatibility:** If NUMERIC → ideal_response has numeric answer? If MATCHING → pairs identified?
...
- **Include `target_question_type` field** matching the assigned question type for Stage 2 processing
