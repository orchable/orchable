...
Specification for the Orchable Community Hub — the platform for sharing, discovering, and remixing AI pipeline assets.
...
#### Scenario: Publish a Prompt Template
...
- **THEN** the system redirects the user to `/login` with a `returnTo` parameter pointing back to the Asset Library
...
#### Scenario: Remix (authenticated user)
- **WHEN** an authenticated user clicks "Remix" on a Hub template
- **THEN** the system clones the template, sets `parent_asset_id = hub_asset.id`, `source_asset_id = hub_asset.source_asset_id ?? hub_asset.id`, and `remix_depth += 1`; the editor opens immediately with the cloned template loaded
