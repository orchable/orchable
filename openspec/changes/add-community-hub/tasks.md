...
- [ ] 4.2 Separate sub-routes `/hub/templates`, `/hub/components`, `/hub/orchestrations`, `/hub/ai-presets` (currently tab-filtered only via `/hub`).
...
- [ ] 6.7 Dedicated `/hub/ai-presets` route (AI Presets currently browsable via tab filter on `/hub`, no standalone route).
